...
static char kTextRefreshScheduledKey;  // 面板：下一轮主线程已排入文字内容模型刷新
static char kTextContentRebuildingKey; // 标签：插件正等值重投当前文字，防止 Hook 自激
...
// A→E 日志确认：点击前后 CommentNewCell、UILabel、_UILabelLayer 及所有导出属性均未变化，
// 但同一 Cell 的用户名、时间和回复会一起恢复；说明旧白底留在 UILabel 未导出的内容模型里，
// 清 _UILabelLayer 或普通 setNeedsDisplay 都触不到它。这里把当前内容做一份等值新对象，再走
// UILabel 的公开 setter，复现点击时抖音重配 Cell 所触发的内容重建；不改文字和任何富文本属性。
static void DKRebuildCommentLabelContentModel(UILabel *label) {
    if (!label) return;

    NSAttributedString *attributedText = label.attributedText;
    NSString *plainText = attributedText ? nil : label.text;
    objc_setAssociatedObject(label, &kTextContentRebuildingKey,
                             @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);

    [CATransaction begin];
    [CATransaction setDisableActions:YES];
    if (attributedText) {
        // initWithAttributedString: 保证新对象同时完整保留附件、段落和交互属性。
        NSAttributedString *rebuilt =
            [[NSAttributedString alloc] initWithAttributedString:attributedText];
        label.attributedText = rebuilt;
    } else if (plainText) {
        // mutableCopy 保证不会因 NSString 的 copy 优化继续复用同一对象。
        label.text = [plainText mutableCopy];
    }

    // setter 若让抖音再次写入白底，会被下面的 UIView Hook 同步拦截；这里再次收敛 opaque。
...
// 下方的精准清理中处理；目标 UILabel 等值重投当前内容，其他文字层只做辅助失效，不改变最终
// 文字、颜色或业务状态。刷新落到下一轮主线程，让抖音本轮布局和文字赋值先收敛；同一个根视图
// 一轮 runloop 内只排一次。
...
static void DKInvalidateCachedTextSurfaces(UIView *view, UIView *scope) {
    ...
    if (DKIsCommentTextBackgroundSurface(view, scope)) {
        DKRebuildCommentLabelContentModel((UILabel *)view);
...
- (void)setBackgroundColor:(UIColor *)color {
    ...
    BOOL rebuildingText = [objc_getAssociatedObject(self, &kTextContentRebuildingKey) boolValue];
    ...
    if (suppressContainer || suppressText) {
        ...
        // 内容重投期间的白底回写是本次 setter 的同步副作用；仍拦成透明，但不再排下一轮，
        // 否则某些 UILabel 子类每次 setter 都写白底时会形成无休止的自激刷新。
        if (!rebuildingText) {
