...
static char kTextRefreshScheduledKey;  // 面板：下一轮主线程已排入文字 backing store 刷新
...
// iOS 27 的 _UILabelLayer 是容器层，真正的文字与光响应内容保存在内部 content/light 子层。
// 只对日志确认的评论标签调用它自己的清理入口；普通 setNeedsDisplay 只能标记外层，无法丢弃
// 已经带上旧白底的内部 backing store。私有入口按类名和 selector 双重守卫，其他 UILabel 不碰。
static void DKInvalidateCommentLabelBackingStore(UIView *view) {
    CALayer *layer = view.layer;
    BOOL clearedInternalContents = NO;
    if ([NSStringFromClass(layer.class) isEqualToString:@"_UILabelLayer"]) {
        SEL clearContents = NSSelectorFromString(@"_clearContents");
        if ([layer respondsToSelector:clearContents]) {
            [CATransaction begin];
            [CATransaction setDisableActions:YES];
            ((void (*)(id, SEL))objc_msgSend)(layer, clearContents);
            [CATransaction commit];
            clearedInternalContents = YES;
        }
    }

    [view setNeedsDisplay];
    [layer setNeedsDisplay];
    if (clearedInternalContents) [layer displayIfNeeded];
}

// 评论标签与 YYText 在背景切换后仍可能保留已栅格化的 backing store。真正的纯白标签底色会在
// 下方的精准清理中处理；这里仅作为清理后的辅助失效，不修改文字、颜色或业务状态。刷新落到
// 下一轮主线程，让抖音本轮布局和文字赋值先收敛；同一个根视图一轮 runloop 内只排一次。
...
static void DKInvalidateCachedTextSurfaces(UIView *view, UIView *scope) {
    ...
    if (DKIsCommentTextBackgroundSurface(view, scope)) {
        DKInvalidateCommentLabelBackingStore(view);
